...
show_help() {
    ...
    echo ""
    echo "注意: 此应用不包含 AI 模型，模型将在首次运行时下载 (~15GB)"
    echo ""
}
...
# 尝试从 main_window.py 提取版本号，如果失败使用默认
VERSION=$(grep -oE "v[0-9]+\.[0-9]+\.[0-9]+" ui/main_window.py 2>/dev/null | head -1 | sed 's/v//' || echo "1.0.0")
...
# 步骤3: PyInstaller 打包
# ============================================
log_step "3" "PyInstaller 打包 (不包含模型)"

log_info "正在打包应用..."
log_info "注意: 模型不包含在应用包中，将在首次运行时下载"
${PYINSTALLER} SuperElite.spec --clean --noconfirm
...
log_success "DMG 创建完成: ${DMG_PATH} (${DMG_SIZE})"
